...
def get_train_loader(dataset, height, width, batch_size, workers,
    ...
    train_loader = IterLoader(
                DataLoader(Preprocessor(train_set, root=dataset.images_dir,
                                        transform=train_transformer, pose=True),
...
def create_model(args, classes):
    model_1 = models.create(args.arch, num_features=args.features, dropout=args.dropout, num_classes=classes, pretrained=False)
    model_2 = models.create(args.arch, num_features=args.features, dropout=args.dropout, num_classes=classes, pretrained=False)

    model_1_ema = models.create(args.arch, num_features=args.features, dropout=args.dropout, num_classes=classes, pretrained=False)
    model_2_ema = models.create(args.arch, num_features=args.features, dropout=args.dropout, num_classes=classes, pretrained=False)
...
def main():
    ...
    if args.dataset_target == "dukemtmc-reid":
        ...
        args.c_dim = 6
    elif args.dataset_target == "lab_data":
        args.c_dim = 3
    else:
...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        ...
        if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):          
            torch.save({'state_dict': model_1_ema.state_dict()}, osp.join(args.logs_dir, 'model_1_checkpoint.pth.tar'))
            torch.save({'state_dict': model_2_ema.state_dict()}, osp.join(args.logs_dir, 'model_2_checkpoint.pth.tar'))
            print(f"Finish epoch {epoch+1}, save model.")
